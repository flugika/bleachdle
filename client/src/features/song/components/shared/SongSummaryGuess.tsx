 ...
 * ร่วมกับโหมดอื่นผ่าน src/shared/ui/summary
 */
export const SongSummaryGuess = ({
    ...
    // 1. เรียก Hook ก่อน Guard Clause ตามกฎของ React
    // 2. ปรับ attemptMatrix ให้เป็น array ช่องเดียวตามสถานะของ Song (ถูก = เขียว / ผิด = แดง)
    const shareData = useShareResultData({
        ...
        dateLabel: mode === 'daily' && scheduledDate
            ? scheduledDate
            : new Date().toISOString().slice(0, 10),
        seed: target?.id ?? 'song-default',
    });
    ...
    return (
        <SummaryCardShell isWin={isWin} kanji={activeTier.kanji} kanjiColor={activeTier.color}>
            ...
            {mode === 'daily' && scheduledDate && (
                <DailyResetTimer targetDate={scheduledDate} />
            )}

            <TierBadgeCard activeTier={activeTier} />

            {/* Target Song Metadata Block */}
