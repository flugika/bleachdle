...
/**
 * 🗨️ เวอร์ชัน quote ของ SummaryGuess — โครงเดียวกับ SongSummaryGuess (tier badge,
 * kanji watermark, accordion history) แต่ "target metadata block" เปลี่ยนจาก
 * title/artist/album เป็น: ตัวคำคม + ตัวละครที่เฉลย (image/name/affiliation)
 * + episode/chapter/arc/context เพราะ BleachQuote ไม่มี field เพลง
 */
            ...
            <div className={`w-full p-6 backdrop-blur-md relative overflow-hidden transition-all duration-500 border ${cardBgStyle}`}>
                ...
                <div className="text-center mb-6 relative z-10">
                    ...
                    <p className="text-[11px] tracking-[0.3em] uppercase text-[#ebc7c7]/50 mt-1">
                        {isWin ? "Voice Signature Resonance Confirmed" : "Voice Link Severed"}
                    </p>
                </div>
                ...
                {/* Target Quote + Answer Reveal Block */}
                <div className="relative mb-6 overflow-hidden border border-[#c8a96e]/20 bg-[#06060a] shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                    ...
                    <div className="px-6 py-6 text-center">
                        <p className="text-[12px] text-[#777796] font-mono tracking-widest select-none">{divider}</p>
                    ...
                    {/* Answer character reveal */}
                    {answerCharacter && (
                        <div className="flex items-center gap-4 px-4 pb-4 pt-2 border-t border-white/[0.06]">
                            <div className="relative h-16 w-16 shrink-0 border border-[#c8a96e]/20 p-[1px] bg-black/40">
                                <Image
                            ...
                            <div className="flex flex-col text-left overflow-hidden">
                                <h2 className="text-lg text-[#f5ebd5] tracking-wide truncate">{answerCharacter.name}</h2>
                                <div className="flex flex-wrap gap-2 mt-1.5">
                                    <span className="px-2 py-0.5 text-[11px] text-[#c8a96e]/80 border border-[#c8a96e]/20 bg-[#c8a96e]/5 font-mono">
                    ...
                    {hasMeta && (
                        <div className="grid grid-cols-2 gap-[1px] bg-[#c8a96e]/10 border-t border-[#c8a96e]/10">
                            {target.episode != null && (
                                <div className="bg-[#0a0a0f]/90 p-3 flex flex-col gap-0.5">
                                    <span className="text-[11px] uppercase tracking-[0.2em] text-[#ebc7c7]/50">Episode</span>
                                    <span className="text-[11px] text-[#ebc7c7]/90 font-medium truncate">{target.episode}</span>
                                </div>
                            )}
                            {target.chapter != null && (
                                <div className="bg-[#0a0a0f]/90 p-3 flex flex-col gap-0.5">
                                    <span className="text-[11px] uppercase tracking-[0.2em] text-[#ebc7c7]/50">Chapter</span>
                                    <span className="text-[11px] text-[#ebc7c7]/90 font-medium truncate">{target.chapter}</span>
                                </div>
                            )}
                            {target.arc && (
                                <div className="bg-[#0a0a0f]/90 p-3 flex flex-col gap-0.5 col-span-2">
                                    <span className="text-[11px] uppercase tracking-[0.2em] text-[#ebc7c7]/50">Arc</span>
                                    <span className="text-[11px] text-[#ebc7c7]/90 font-medium truncate">{target.arc}</span>
                                </div>
                            )}
                            {target.context && (
                                <div className="bg-[#0a0a0f]/90 p-3 flex flex-col gap-0.5 col-span-2">
                                    <span className="text-[11px] uppercase tracking-[0.2em] text-[#ebc7c7]/50">Context</span>
                                    <span className="text-[11px] text-[#ebc7c7]/90 leading-relaxed">{target.context}</span>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="my-4 border-t border-white/[0.05] pt-4 flex flex-col items-center w-full">
                    ...
                    {/* Matrix Squares — quote มีแค่ correct/wrong เหมือน song เลยเหลือสี่เหลี่ยมเดียวต่อการเดา */}
