...
const GuessRow = ({ guess, result, isNew = false }: GuessEntry) => {
    ...
    return (
        // 🎯 FIX: ใช้ Plain <div> มารับหน้าที่สั่นและรับ ID แทน เพื่อหนีจากทรานสิชันของ Framer Motion
        <div
            id={`row-${guess.id}`}
        ...
        >
            ...
            >
                ...
                <div className="bg-[#111120] border border-[#1e1e30] overflow-hidden w-[72px] h-[72px] flex-shrink-0">
                    <img
                ...
                <ResultCell value={formatHeight(guess.height_cm)} status={result.height} colIndex={3} animate={isNew} />
                <ResultCell value={formatAge(guess.age)} status={result.age} colIndex={4} animate={isNew} />
                <ResultCell value={guess.eye_color} status={result.eye_color} colIndex={5} animate={isNew} />
                <ResultCell value={guess.hair_color} status={result.hair_color} colIndex={6} animate={isNew} />
                <ResultCell value={guess.weapon.join(' , ')} status={result.weapon} colIndex={7} animate={isNew} />
                <ResultCell value={guess.primary_ability.join(' , ')} status={result.primary_ability} colIndex={8} animate={isNew} />
                <ResultCell value={guess.release.join(' , ')} status={result.release} colIndex={9} animate={isNew} />
                <ResultCell value={guess.first_appearance_chapter} status={result.first_appearance_chapter} colIndex={10} animate={isNew} />
            </motion.div>
        </div>
    );
};
