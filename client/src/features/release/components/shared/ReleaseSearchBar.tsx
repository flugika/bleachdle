// src/features/release/components/shared/ReleaseSearchBar.tsx
'use client';
...
interface ReleaseSearchBarProps {
...
export const ReleaseSearchBar = ({ releases, disabled = false, game }: ReleaseSearchBarProps) => {
    ...
    // รวบรวม ID release ที่เคยเดาไปแล้วเพื่อป้องกันการส่งซ้ำ
    const guessedIds = useMemo(
        () => new Set(guesses.map(g => g.guess.id)),
        [guesses]
    );

    // 🔥 ค้นหาได้ทั้งจาก technique_name (คำตอบหลัก, โชว์เป็นหลัก) และ technique_translation
    // (ความหมายอังกฤษ, โชว์เป็น subtitle) — เผื่อผู้เล่นจำได้แค่ชื่ออังกฤษไม่ใช่ชื่อ romanized
    const searchEngine = useMemo(
        () => createSearchEngine(releases, {
    ...
    const triggerScrollAndShake = (releaseId: string) => {
        const rowEl = document.getElementById(`release-row-${releaseId}`);
        if (!rowEl) return;

        rowEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        rowEl.classList.add('animate-reiatsu-shake', 'ring-1', 'ring-red-600/70', 'z-20');

        setTimeout(() => {
            rowEl.classList.remove('animate-reiatsu-shake', 'ring-1', 'ring-red-600/70', 'z-20');
        }, 600);
    };

    const handleSelect = (release: BleachRelease) => {
        if (guessedIds.has(release.id)) {
            triggerScrollAndShake(release.id);
        } else {
            addGuess(release.id);
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0 pr-4 transition-transform duration-200 group-hover:translate-x-1">
                                    ...
                                    {item.technique_translation && (
                                        <span className={[
                                            'text-[12px] mt-0.5 tracking-wide font-medium truncate transition-colors duration-200 italic',
                                            isActive ? 'text-white/60' : 'text-[#555566]'
                                        ].join(' ')}>
                                            &ldquo;{item.technique_translation}&rdquo;
                                        </span>
                                    )}
                                </div>

                                {/* BADGE STATE — release_type ก่อน guessed override */}
                                {isGuessed ? (
                                    ...
                                    <span className="ml-3 shrink-0 text-[9px] tracking-[0.18em] font-bold uppercase text-[#777796] border border-[#3d3d54]/60 px-1.5 py-0.5">
                                        {item.release_type}
                                    </span>
                                )}
                            </li>
